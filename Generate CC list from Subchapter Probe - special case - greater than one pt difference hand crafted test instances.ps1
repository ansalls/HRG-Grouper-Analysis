...
# Initial input file
$StartFolder = Join-Path $BaseDir 'data\hrg_output\mini\Max Score Needed  GT 1'
...
& (Join-Path $BaseDir 'col_substring_compare.exe') -n $InputFile $OutputFile 226 260
Log-Step -Step $Step -Timer $timer
...
& (Join-Path $BaseDir 'col_substring_compare.exe') -l 4 $InputFile $OutputFile 226 260
Log-Step -Step $Step -Timer $timer
