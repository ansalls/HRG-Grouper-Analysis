...
class EpisodeDuration(Probe):
    ...
    def probe_values(cls) -> list[int]:
        '''
            Idea here is to take all days up to ~month, then 5 days up to 150,
            then 25 days up to 500, then 500 days up to 1000...

            This will provide enough granularity around likely cutoff points
            to dig deeper into those points without having to probe every single
            possible value between 0 and MAX_EPISODE_DURATION.
        '''
